    ...
    def generate_caption(self, image_path: str) -> str:
        try:
            ...
            if self.detailed:
                ...
                return self._deduplicate_words(caption.strip())
        except Exception as e:
...
class DatasetPreparer:
    ...
    def process_directory(self, input_dir: Path, output_dir: Path,
        ...
        # Use resolve().as_posix() for cross-platform path normalization
        seen_files = set()
        image_files = []
        ...
        for pattern in ['*.jpg', '*.jpeg', '*.png', '*.JPG', '*.JPEG', '*.PNG']:
            for file_path in input_dir.glob(pattern):
                ...
                # Use resolve().as_posix() to handle Windows/Linux path differences consistently
                try:
